...
# For GreenThreading not I don't know how it is behaving with MultiProcessing/Threading
...
class SCSpider(Process):
    ...
    def __init__(self, *sc_queues, **kwargs):
        ...
        if self.headers['User-Agent'] is None:
        ...
        self.xtrees_q = kwargs.pop("xtrees_q", Queue())

        # Use external Queue only for Interprocess Communication if any
        ...
        # urls_number_stop: Stop in a Default Values (if none given from user) for...
        # ...Politeness and because there is no point to have more samples of this site (I think)
    ...
    def run(self):
        ...
        # Use the netloc (network locator) of the seed url as the base URL that this spider...
        # ...is working on
        url = self.urls_l[0]
        ...
        # A thread is constantly checking the DUE seen dictionary is big enough to...
        # ...be saved on disk
        ...
        scanned_urls = 1
        # Counter for the URLS that have been Followed by the Crawler - SHOULD BE HERE
        ...
        while True:
            ...
            if self.urls_l == []:
                ...
                print(
                    "SCSpider Process (PID = %s - PCN = %s): Terminated" +
                    " - No more URL links to follow" % (self.pid, self.pnum)
                )
            ...
            # Get one of the URLs which an other SCSpider has left to the ext_due_p...
            # ...and append it to the urls_l
            if self.ext_url_q is not None:
            ...
            # Start Processing WebPages (in fact sockets to them) which a Pool of...
            # ...GreenThreads is harvesting Asynchronously.
            for xhtml in n_gpool.imap(self.fetchsrc, self.urls_l):
                ...
                if xhtml[0]:
                    ...
                    if scanned_urls >= self.urls_number:
                        print(
                            "SCSpider Process (PID = %s - PCN = %s):" +
                            " Terminated - User Condition: Stop On %d Pages" +
                    ...
                    for link in self.link_extro.sites_links(xhtml_d):
                        ...
                        baseurl = str(
                            self.due.base_url['scheme'] + "://" + self.due.base_url['netloc']
                        )
                    ...
                    print(
                        "SPIDER %d of %d BASE %s"
                        % (self.pnum, SCSpider.Num, self.due.base_url['netloc'])
                    )
                    print("Empty Page: %s" % xhtml[1])

            # Now give the new URLs List back to the Fetcher GreenThreads
            del self.urls_l
            # for Preventing Memory Leakage remove it if has no effect but delay of the inevitable

            # print len(self.urls_l)
            self.urls_l = tmp_urls_l
            print len(self.urls_l)

        # If this Process has to be terminated wait for the Disk_keeper Thread to...
        # ...finish its job and join
        self.due.acquire()
    ...
    def save_xhtml(self, xhtml_d):
        ...
        try:
            ...
            try:
                file = self.save_path + str(xhtml_d['netloc']) + "." +\
                    str(self.file_counter) + ".html"
                f = os.open(
                    file,
                    os.O_CREAT | os.O_WRONLY, stat.S_IWUSR |
                    stat.S_IWGRP | stat.S_IWOTH | stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH
                )
            except Exception as e:
            ...
            # Place an Encoding Wrapper to assure the file Writing to be performed...
            # ...with UTF-8 encoding.
            # if xhtml_d['charset']:
            ...
            # Return None for the Spider to know that some error occurred for...
