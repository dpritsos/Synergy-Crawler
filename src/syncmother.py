...
class SynCMother:
    """symmother Class is handling two internal dictionaries one for web Sites that pending to be scanned and downloaded and
    one for the ready web sites, i.e. the ones that already been given to a process Pool to scan and download them""" 
    def __init__(self):
        self.rawUrlsQ = Queue(100)
        hash = hashlib.md5()
        hash.update("http://www.google.gr/search?q=google&hl=el&client=firefox-a&hs=ksj&rls=com.ubuntu:en-GB:official&prmd=n&source=lnms&tbs=nws:1&ei=hhUuTPeRJML58Aa-_-C7Aw&sa=X&oi=mode_link&ct=mode&ved=0CBIQ_AU")
        self.pending_urls = [[hash.digest(), "http://www.google.gr/search?q=google&hl=el&client=firefox-a&hs=ksj&rls=com.ubuntu:en-GB:official&prmd=n&source=lnms&tbs=nws:1&ei=hhUuTPeRJML58Aa-_-C7Aw&sa=X&oi=mode_link&ct=mode&ved=0CBIQ_AU"]] #None 
        self.scanned_urls = None 
        self.ready_urls = threading.Condition() #The conditional variable notify the synworkers that some URL pending for handling
